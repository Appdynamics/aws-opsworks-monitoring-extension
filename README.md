# AWS OpsWorks Monitoring Extension

## Use Case
Captures OpsWorks statistics from Amazon CloudWatch and displays them in the AppDynamics Metric Browser.
...
1. Run 'mvn clean install' from aws-opsworks-monitoring-extension
2. Copy and unzip AWSOpsWorksMonitor-\<version\>.zip from 'target' directory into \<machine_agent_dir\>/monitors/
3. Edit config.yml file in AWSOpsWorksMonitor/conf and provide the required configuration (see Configuration section)
...
1. If SIM is enabled, then use the following metricPrefix - 
   ...
   `metricPrefix: "Custom Metrics|Amazon OpsWorks|"`
   ...
   `metricPrefix: "Server|Component:<COMPONENT_ID>|Custom Metrics|Amazon OpsWorks|"`.

   For example,
         
    ```
    metricPrefix: "Server|Component:100|Custom Metrics|Amazon OpsWorks|"
    ```
...
3. If you want to encrypt the **awsAccessKey** and **awsSecretKey** then follow the "Credentials Encryption" section and provide the encrypted values in **awsAccessKey** and **awsSecretKey**. Configure `enableDecryption` of `credentialsDecryptionConfig` to `true` and provide the encryption key in `encryptionKey`.
   ...
   ```
4. To report metrics only from specific dimension values, configure the `dimesion` section. Dimensions for AWS OpsWorks are `StackId, LayerId and InstanceId`. 
    ```
    dimensions:
      - name: "StackId"
        displayName: "StackId"
...
### Config.yml
Please avoid using tab (\t) when editing yml files. Please copy all the contents of the config.yml file and go to [Yaml Validator](http://www.yamllint.com/) . On reaching the website, paste the contents and press the “Go” button on the bottom left.                                                       
If you get a valid output, that means your formatting is correct and you may move on to the next step.

## Metrics
Typical metric path: **Application Infrastructure Performance|\<Tier\>|Custom Metrics|Amazon OpsWorks|\<Account Name\>|\<Region\>|\<Stack or Layer or Instance\>|\<instance\>** followed by the metrics defined in the link below:

- [OpsWorks Metrics](http://docs.aws.amazon.com/AmazonCloudWatch/latest/DeveloperGuide/ops-metricscollected.html)
...
Always feel free to fork and contribute any changes directly via [GitHub](https://github.com/Appdynamics/aws-opsworks-monitoring-extension).
